...
- By specifying a different model -- namely, an autoregressive one -- we may have fixed the problem of autocorrelation 
...
where $\rho$ (*RHO*) is the AR(1) parameter.
...
fed_tbl$panel = 1
fixest::feols(gdp~gov, fed_tbl, se="NW", panel.id=~panel+year)
```